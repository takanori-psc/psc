#!/usr/bin/env python3
"""Minimal LIGHT telemetry-gap scenario stub.

This stub only proves the current scenario intent:
missing required LIGHT telemetry holds the recovery ramp instead of advancing it.
"""
...
def decide_light_ramp_step(step: int, sample: dict[str, object]) -> str:
    ...
    if missing:
        ...
        print(
            ...
            "ramp_state=RAMPING "
            f"scenario=light_telemetry_gap missing={','.join(missing)} "
    ...
    print(
        ...
        "observation_mode=LIGHT "
        "ramp_state=RAMPING "
        "scenario=light_telemetry_gap reason=NO_TELEMETRY_GAP_CASE"
    )
    return "RULE-22_RETURN_RAMP_HOLD"
...
def run() -> None:
    telemetry_sample = {
        "trust": 0.95,
        # stability_proxy intentionally omitted.
        "freshness": "current",
        "confidence": "accepted",
    }
