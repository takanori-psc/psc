...
# trust-fixed case broken version
# =========================================

POLICY = "latency"   # "stability" or "latency"
TRUST_MODE = "require_trusted"   # "off", "prefer_trusted", "require_trusted"
...
# -----------------------------------------
# Trust table
# v10a: nodeD also becomes UNTRUSTED
# -----------------------------------------
trust_table = {
    "nodeA": True,
    "nodeB": True,
...
def select_route_with_policy_and_resolver(current_route, best_route, best_cost, node_states, policy):
    ...
    if best_route is None:
        current_cost = calculate_route_cost(current_route, node_states) if current_route else None
        return {
            "selected_route": current_route,
            "selected_cost": current_cost,
            "decision": "NO_ROUTE",
            "current_cost_now": current_cost,
...
def main():
    ...
    print("=== Policy-aware Routing Simulation v10a ===")
    ...
    for step_index, node_states in enumerate(simulation_steps, start=1):
        ...
        print(f"Trusted candidates: {len(trusted_paths)}")
        print(f"Untrusted candidates: {len(untrusted_paths)}")
