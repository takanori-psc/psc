...
| RULE-08_DEGRADE_keep | Unnecessary fallback switching during DEGRADED mode | Keep a stable path even in degraded mode | degraded_keep | `sim/02_controlled/02_rcu_decision_v01/logs/rcu_decision_v01_degraded_keep_experiment_log.md` | Verified |
...
| RULE-20_RETURN_KEEP | Risk of switching to a return candidate before conditions are met | Keep the current path | recovery_return_v02 / recovery_ramp_v03 | `sim/02_controlled/06_recovery_return_v02/mini_psc_rcu_decision_v03_recovery_ramp_observation.py` | Implemented |
...
1. Add a dedicated evidence log for `RULE-02_SWITCH_score`.
2. Organize the existing evidence for `RULE-08_DEGRADE_keep` and `RULE-20_RETURN_KEEP` as dedicated validation logs.
3. Keep v0.3 RULEs as Experimental until they are integrated into the specification.
4. Add the relevant step / line references for each evidence log in this matrix.
