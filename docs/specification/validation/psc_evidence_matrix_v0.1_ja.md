...
| RULE-08_DEGRADE_keep | DEGRADED 中の不要な fallback 切替 | degraded mode でも安定 path を維持 | degraded_keep | `sim/02_controlled/02_rcu_decision_v01/logs/rcu_decision_v01_degraded_keep_experiment_log.md` | Verified |
...
| RULE-20_RETURN_KEEP | 復帰候補が条件未達のまま切替される危険 | current path 維持 | recovery_return_v02 / recovery_ramp_v03 | `sim/02_controlled/06_recovery_return_v02/mini_psc_rcu_decision_v03_recovery_ramp_observation.py` | Implemented |
...
1. `RULE-02_SWITCH_score` の専用証拠ログを追加する。
2. `RULE-08_DEGRADE_keep` と `RULE-20_RETURN_KEEP` は、現在の証拠を専用 validation log として整理する。
3. v0.3 系 RULE は Experimental のまま、仕様統合後に Verified へ昇格する。
4. 本マトリクスの各証拠ログに対して、該当 step / line を追記する。
