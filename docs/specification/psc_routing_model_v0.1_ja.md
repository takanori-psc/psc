...
- ドキュメント名: PSC Routing Modelバージョン : v0.1
...
PSC Routing Model は、PSC Fabric 上で PSC ノード、ポート、およびクラスタ間の転送経路がどのように決定されるかを説明する。

また、トポロジー、障害、負荷状況、および将来的な多層ファブリック拡張に対して、ルーティング制御がどのように適応するかを定義する。

本仕様は主に、論理的なルーティングアーキテクチャおよび RCU（Routing Control Unit）の制御動作に焦点を当てる。
...
フェイルオーバールーティングは、リンク・ノード・トポロジー障害が発生した場合でも通信を継続可能にすることを目的とする。

### 9.2 トリガー条件

フェイルオーバールーティングは以下の条件で発生する。
...
フェイルオーバーが発生した場合、RCU は以下を行う。

1. 無効または危険な経路を無効化する  
2. 代替経路を探索する  
3. ポリシーおよびセキュリティ制約を維持する  
4. TMU にルート変更を通知する  
5. 不安定なルート振動を可能な限り回避する  

### 9.4 段階的劣化（Graceful Degradation）

優先ルートが利用できない場合、PSC は即座に通信を停止するのではなく、段階的劣化を行う。
