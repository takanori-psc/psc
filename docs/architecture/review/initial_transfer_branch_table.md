...
| NORMAL_TRANSFER / initial forwarding state | Score / trust switch candidate points to an unsafe path, an explicit trust-blocked path, or a below-threshold trust path | RULE-04_BLOCK_trust | NORMAL_TRANSFER / BLOCK_SWITCH | Verified safety block. Explicit trust_block and below-threshold trust are separate causes, but both produce `BLOCK_SWITCH` and take priority over `RULE-02_SWITCH_score` and `RULE-03_SWITCH_trust` |
